...
Future<void> main() async {
  print('====================================================');
  print('Initializing mWallet Database Tables on Remote Gateway');
  print('Target DB: ${SyncConfig.database}');
  print('Gateway: ${SyncConfig.apiGatewayUrl}');
  print('API Key: ${SyncConfig.apiKey}');
  print('====================================================\n');

  for (int i = 0; i < createTableQueries.length; i++) {
    final query = createTableQueries[i];
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        if (data['success'] == true) {
          print('  [✓] Table ${i + 1}/${createTableQueries.length} created successfully.');
        } else {
          print('  [✗] Table ${i + 1} query error: ${data['error']}');
        }
      } else {
        print('  [✗] Table ${i + 1} HTTP Error: ${resp.statusCode} - ${resp.body}');
      }
    } catch (e) {
      print('  [✗] Table ${i + 1} Exception: $e');
    }
  }
